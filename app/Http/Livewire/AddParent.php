...
class AddParent extends Component
{
            ...
            if (!empty($this->photos)) {
                foreach ($this->photos as $photo) {
                    ...
                    ParentAttachment::create([
                        ...
                        'parent_id' => my__Parent::latest()->first()->id,
                    ]);
                }
            }
    ...
    public function submitForm_edit()
    {
        ...
        if ($this->Parent_id) {
            ...
            $My_Parent = my__Parent::find($this->Parent_id);
            // Father_INPUTS
            $My_Parent->update([
                'Email' => $this->Email,
    ...
    public function delete($id)
    {
        ...
        try {
            if (!empty(ParentAttachment::where("parent_id", $id))) {
                ParentAttachment::where("parent_id", $id)->delete();


                Storage::disk('parent_attachments')->deleteDirectory(my__Parent::findOrFail($id)->first()->National_ID_Father);
            }

            my__Parent::findOrFail($id)->delete();
            return redirect()->to('/add_parent');
        } catch (\Exception $e) {
            $this->catchError = $e->getMessage();
        };
    ...
    public function edit($id)
    {
        $this->show_table = false;
        $this->updateMode = true;
        $My_Parent = my__Parent::where('id', $id)->first();
        $this->Parent_id = $id;
